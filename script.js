...
function setFocusInfo(focusKey) {
  ...
  if (focusKey === 'space') {
    ...
    focusPageMessage = 'This video shows that moment that the payload reached the edge of space and the balloon burst, sending the payload hurtling back to Earth.';
    focusUrl = 'https://www.youtube.com/embed/9Rz9ZmfYuqk?rel=0&autoplay=1';
    focusTitle = 'Edge of Space Explorer';
    focusPara1 = 'I was privileged to have the amazing oportunity of sending a Raspberry Pi computer to the edge of space attached to a high-altitude balloon as a primary school anniversary celebration project. The payload included a range of cameras programmed in python to record a mix of video and images and send live images back to Earth, a GPS unit to track the journey and transmitter to send back live telemetry of the flight.';
    focusPara2 = 'On the ground, we chased the flight in a minibus, making live predictions about the landing site so we could quickly retrieve the payload as it came hurtling back to Earth and landed safely in a wheat field.';
  } else if (focusKey === 'forest') {
    focusPageMessage = 'Pan around the 360 degree view of March Wood including right up to the sky and down to the ground. This image was taken when March Wood was just a couple of years old.';
    focusUrl = 'https://pablisch.github.io/MarchWood/';
    focusTitle = 'Forest Creator';
    focusPara1 = 'The greatest barrier to setting up Forest School at my primary school was the profound lack of trees so I planted a brand new baby woodland that become known as March Wood. There was no end of challenges including funds and resources, Immovable deadlines, the reality of how long natures take to grow what you want as well as how quickly it grows what you definitely do not want, and helping saplings survive the long, hot summer holidays when the school is closed.';
    focusPara2 = 'It took passion, dedication and resilience, especially in the midst of the pandemic lockdowns, but March Wood has thrived, tiny saplings now tower over me and we have an amazing outdoor classroom for all the school to enjoy, not to mention and incredible legacy woodland.';
  } else if (focusKey === 'stem') {
    focusPageMessage = 'The robots we used were designed, built and programmed by the children that drove them.';
    ...
    focusTitle = 'Furniture Designer & Maker';
...
function loadFocusInfo() {
  document.querySelector('#focus-title').innerHTML = focusTitle;
